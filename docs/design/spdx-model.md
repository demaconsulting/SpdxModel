...
- `Spdx2JsonDeserializer.Deserialize` throws `System.Text.Json.JsonException` when the input
  is fatally malformed JSON that cannot be parsed. Missing or unknown SPDX fields do not throw;
  they produce default or empty values in the resulting `SpdxDocument`.
- `SpdxDocument.Validate(List<string>)` never throws. It appends human-readable issue strings
  to the supplied list; an empty list after the call indicates a valid document.
